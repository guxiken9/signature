...
| `options.width` | integer | - | リサイズ後の画像の幅（ピクセル、最小値: 1） |
| `options.height` | integer | - | リサイズ後の画像の高さ（ピクセル、最小値: 1） |
...
`width` と `height` を指定することで、画像をリサイズできます。アスペクト比は自動的に維持されます。
...
**処理順序**:
1. 透明ピクセルのトリミング（`trimTransparent: true` の場合）
2. 画像リサイズ（`width` または `height` が指定された場合）
3. 背景色の適用
4. フォーマット変換
...
**注意事項**:
- 最小値は1ピクセル
- リサイズには高品質な補間アルゴリズム（BICUBIC）を使用
